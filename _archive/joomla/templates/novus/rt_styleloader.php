...
//load font style
if ($session->get($cookie_prefix. 'fontstyle')) {
    $fontstyle = $session->get($cookie_prefix. 'fontstyle');
} elseif (isset($_COOKIE[$cookie_prefix. 'fontstyle'])) {
	$fontstyle = JRequest::getVar( $cookie_prefix.'fontstyle', '', 'COOKIE', 'STRING');
}
...
if ($session->get($cookie_prefix. 'mtype')) {
    ...
    $mtype = JRequest::getVar( $cookie_prefix.'mtype', '', 'COOKIE', 'STRING');
}
